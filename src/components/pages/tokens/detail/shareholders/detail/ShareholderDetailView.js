              ...
              <Table.Body>
                {(shareholder.ethAddresses || []).map(ethAddress => {
                  return <Table.Row key={ethAddress.address}>
                    <Table.Cell>
                      <Link
                        to={`https://${window.REACT_APP_APP_TYPE ? '' : 'kovan.'}etherscan.io/address/${ethAddress.address}`}
                        target='_blank'
                    ...
                    <Table.Cell>{(ethAddress.issues || []).map(issue => {
                      return <p key={issue.address}>
                        ...
                        <Link
                          to={`https://${window.REACT_APP_APP_TYPE ? '' : 'kovan.'}etherscan.io/address/${issue.address}`}
                          target='_blank'
