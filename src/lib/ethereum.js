...
export default {
  init({
    walletHost = process.env.REACT_APP_WALLET_HOST ||
      'https://kovan.infura.io/V7nB2kBfEei6IhVFeI7W',
    ...
    if (window.web3 && window.web3.currentProvider) {
      ...
      providerEngine.addProvider(
        new Web3Subprovider(
          new Web3.providers.HttpProvider(`${walletHost}:${walletPort}`)
        )
      )
    }
      ...
      providerEngine.addProvider(
        new Web3Subprovider(
          new Web3.providers.HttpProvider(
            'https://kovan.infura.io/O4y6ossOQVPXYvf8PDB4'
          )
        )
      ) // TODO: implement redundantRPC
    }
    ...
    return web3.eth
      ...
      .catch(error => {
        console.error(
          `Error connecting to wallet on host ${walletHost}:${walletPort}, error: ${error}`
        )
  ...
  addInvestorToWhitelist(investorAddress, contractAddress) {
    const contract = web3.eth
      .contract(whitelistContract.abi)
      .at(contractAddress)
    promisifyAll(contract.add)
    return contract.add.sendTransactionAsync(investorAddress, {
      from: currentAccount,
    })
  },
  ...
  setRegDWhitelistReleaseDate(investorAddress, contractAddress, releaseDate) {
    const contract = web3.eth
      .contract(regDWhitelistContract.abi)
      .at(contractAddress)
    promisifyAll(contract.setReleaseDate)
    return contract.setReleaseDate.sendTransactionAsync(
      investorAddress,
      releaseDate,
      { from: currentAccount }
    )
  ...
  setMessagingAddress(messagingAddress, tokenAddress) {
    const deployedTokenContract = web3.eth
      .contract(tokenContract.abi)
      .at(tokenAddress)
    promisifyAll(deployedTokenContract._service)

    return deployedTokenContract._service
      .callAsync()
      .then(regulatorServiceAddress => {
        const deployedRegulatorServiceContract = web3.eth
          .contract(regulatorServiceContract.abi)
          .at(regulatorServiceAddress)
        ...
        return deployedRegulatorServiceContract.getMessagingAddress
          .callAsync()
          .then(currentMessagingAddress => {
            if (currentMessagingAddress !== messagingAddress) {
              return deployedRegulatorServiceContract.setMessagingAddress.sendTransactionAsync(
                messagingAddress,
                { from: currentAccount }
              )
            }
